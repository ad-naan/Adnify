...
### ✨ 新功能 / 优化
- 🛡️ **安全白名单默认值收敛**
  - 将安全白名单默认值收敛为单一共享配置来源，消除主进程安全模块与设置默认值的重复定义和漂移。
  - 启动和保存 `securitySettings` 时会自动标准化旧白名单，补齐历史缺失的 `git notes`、`merge-base`、`symbolic-ref` 等子命令。
- 🧭 **多模态模型路由**
  - 新增 `modelRouting` 配置，支持为图片请求单独指定多模态模型，同时保持当前主模型为默认对话入口。
  - 只有最新一轮用户消息包含图片时才触发视觉预处理，避免历史图片在后续轮次被重复分析。
- 🖼️ **视觉摘要接力**
  - 多模态模型会先生成结构化视觉摘要，再把摘要和原始用户请求一起交给主模型继续对话与工具调用。
  - 预处理失败时会移除图片内容并回退到主模型，同时在界面里给出提示，保证旧链路仍可用。
- ⚙️ **设置与持久化**
  - 设置页新增多模态提供商和模型选择，并展示当前主模型。
  - `modelRouting` 支持默认值初始化、保存恢复、导入导出，以及 onboarding 首次配置写入。
- 🛡️ **配置清理**
