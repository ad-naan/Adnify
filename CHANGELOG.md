...
## [1.7.69] - 2026-09-18 扩展管理、计划工作台与流式推理增强

> **版本亮点**：引入 Agent 管理的扩展事务与凭据处理系统，支持 MCP/Skill 扩展的搜索、安装与卸载；计划工作台全面升级，新增展示钩子、历史抽屉和任务审查；流式推理新增长思考不活跃超时机制；聊天中支持文件路径识别与跳转；ZIP 提取加固防止符号链接攻击；新增多语言 README 翻译（日韩西法德葡俄）。

### 扩展与配置管理 / Extension and Configuration Management
- **Agent 管理的扩展事务**
...
### 流式推理与模型管理 / Streaming and Model Management
- **长思考不活跃超时**
...
### 聊天与文件路径 / Chat and File Paths
- **文件路径识别与跳转**
  - 聊天组件新增文件路径处理，支持嵌套文件和边缘情况的路径解析与识别。
  - 新增 `ChatFilePathBoundary` 组件和 `chatFilePaths` 工具，支持从聊天消息中识别并打开文件。
  - 新增 `hasAsciiControlCharacters` 工具函数，集成到文件路径校验中防止异常字符。
  - 文件路径操作增强错误处理和本地化提示。
- **聊天界面优化**
  - 改进聊天组件的布局处理和溢出管理。

### 工具与 Agent 逻辑 / Tools and Agent Logic
- **循环与反馈改进**
  - 增强循环处理与反馈机制，改进提示词校验，更新工具路由逻辑。
- **国际化**
  - 更新中英文语言文件，移除过时提示词并优化消息表述。

### 工作台布局与 Git / Workbench Layout and Git
- **Dock 工作台调整**
  - 增强 `DockWorkbench` 缩放功能，改进状态管理和 CSS 调整。
- **Worktree 改进**
  - 改进 Worktree 分支创建及未提交变更的错误处理。

### 安全加固 / Security Hardening
- **ZIP 提取安全增强**
  - ZIP 提取使用 `@electron-internal/extract-zip`，拒绝归档遍历和预存在的符号链接。
  - 新增符号链接替换处理：提取时替换预存在的文件符号链接，不覆盖外部文件。
  - 新增依赖补丁测试验证提取安全行为。

### 文档与多语言 / Documentation and Multilingual
- **多语言 README**
  - 新增日语、韩语、西班牙语、法语、德语、葡萄牙语（巴西）、俄语翻译版本。
  - 更新 README.md 和 README_CN.md 添加多语言版本链接。

### 依赖与构建 / Dependencies and Build
- 更新 `pnpm-workspace.yaml` 中 hono、js-yaml、qs、sharp 依赖覆盖至最新兼容版本。
- `vite.config.ts` 新增 `@electron-internal/extract-zip` 为运行时外部依赖。
- 本地化对等测试排除测试文件的引用字面量收集。

---

## [1.7.68] - 2026-09-07 工作台布局自定义与终端体验改进

> **版本亮点**：支持调整工作台面板顺序、比例与终端停靠位置，并按工作区记住布局。修复非交互命令失败时诊断输出被覆盖的问题，新增终端标签右键菜单、归档一键清理和可配置的自动回收策略，同时优化后台及被遮挡区域的装饰动画。

### 工作台布局 / Workbench Layouts
- **按工作习惯调整面板与终端**
...
### 终端标签与输出归档 / Terminal Tabs and Output Archives
- **终端标签右键管理**
...
### 终端执行与诊断 / Terminal Execution and Diagnostics
- **保留失败命令的诊断输出**
...
### 界面性能与质量 / Interface Performance and Quality
- **减少无效动画与完善界面一致性**
...
## [1.7.67] - 2026-09-06 执行管理、通知推送与多设备预览

> **版本亮点**：新增执行管理器，集中查看跨窗口命令、后台服务和日志，支持服务托管与资源限额设置；任务完成、失败和待审批可通过系统通知或 Webhook 提醒。内嵌预览新增手机、平板与横竖屏切换，并按项目隔离登录态。后台任务支持任务栏进度、防休眠和唤醒后连接检查，同时新增性能诊断，将索引、存储与内容解析迁入独立进程。

### 执行管理与后台服务 / Execution Management and Background Services
- **集中管理命令、服务与终端会话**
...
### 系统通知与外部推送 / System Notifications and Webhooks
- **按事件选择任务提醒**
...
### 多设备预览与项目会话隔离 / Device Preview and Project Session Isolation
- **桌面、手机和平板预览**
...
### 后台任务与性能诊断 / Background Tasks and Performance Diagnostics
- **任务栏进度与唤醒后的连接检查**
...
### 进程隔离与设置整理 / Process Isolation and Settings Organization
- **索引、存储与内容解析迁入独立进程**
...
## [1.7.66] - 2026-09-04 自定义素材能力、内嵌浏览器自动化与会话稳定性改进

> **版本亮点**：新增可接入自定义 API 的素材能力，让 Agent 生成并管理图片、视频、音频和文件；内嵌浏览器支持页面检查与交互，方便直接验证前端效果。工具详情统一为紧凑的无边框样式，子任务审批整合进现有 Dock，同时修复会话切换重放、首次用户消息空白和工具收纳时的滚动跳动。上下文压缩、代码诊断、终端与桌面运行时也得到改进。

### 自定义素材能力 / Custom Asset Capabilities
- **接入自己的素材服务**
...
### 内嵌浏览器与页面验证 / Embedded Browser and Page Verification
- **Agent 可直接检查和操作页面**
...
### 聊天与工具交互 / Chat and Tool Interaction
- **统一紧凑的无边框工具详情**
...
### 上下文与代码诊断 / Context and Code Diagnostics
- **上下文压缩与会话接续更可靠**
...
### 桌面运行时与稳定性 / Desktop Runtime and Stability
- **桌面依赖升级与打包修复**
