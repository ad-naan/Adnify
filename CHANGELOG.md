...
## [1.7.70] - 2026-09-23 交互决策工具、拟物思考旋钮与浏览器拟人化自动化

> **版本亮点**：引入 ask_user 核心决策工具，支持结构化可选项一键确认；思考强度全面重构为 270° 拟物金属滚花旋钮并完善 ARIA 无障碍访问；浏览器自动化新增元素 ID 支持与拟人化贝塞尔曲线鼠标交互；上下文压缩增强关键决策与用户约束追踪；优化流式期间 GPU 与终端性能，扩充多维度系统压测套件。

### Agent 与交互决策 / Agent and Interaction Decisions
- **结构化决策工具 ask_user**
...
### UI 与交互体验 / UI and User Experience
- **拟物金属旋钮思考强度调节器**
...
### 浏览器自动化与预览 / Browser Automation and Preview
- **元素定位与拟人化鼠标交互**
...
### 性能与系统工程 / Performance and System Engineering
- **流式渲染与终端输出性能优化**
...
> **版本亮点**：引入 Agent 管理的扩展事务与凭据处理系统，支持 MCP/Skill 扩展的搜索、安装与卸载；计划工作台全面升级，新增展示钩子、历史抽屉和任务审查；流式推理新增长思考不活跃超时机制；聊天中支持文件路径识别与跳转；ZIP 提取加固防止符号链接攻击；新增多语言 README 翻译。
