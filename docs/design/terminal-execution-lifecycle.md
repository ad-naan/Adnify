...
状态：核心执行路径已实现；本文保留总体设计及后续扩展边界。日期：2026-09-05。

当前实现：普通本地命令使用独立进程；主进程按窗口/任务公平排队，普通命令与后台服务分开计量；持久终端使用主进程租约；停止确认后释放名额；命令结果未知时禁止自动重跑。窗口刷新可重新取得运行作业、日志和交互终端，真实窗口关闭由主进程清理。显式传入 `service_key` 可复用同一工作区、相同配置的本地后台服务，最后一个使用它的窗口关闭时停止。

实现边界：容量目前使用共享代码默认值，尚无设置界面；日志当前使用有界内存并恢复 renderer 刷新，不提供退出应用后的磁盘日志归档；交互会话保留 shell 状态，不自动销毁未知或人工持有的会话；不提供所有窗口关闭后继续托管服务的开关。远程 SSH 关闭仅确认连接关闭，不保证远端子进程停止。下面关于磁盘归档、可配置容量、空闲缓存和独立工作区服务托管的条目是进一步扩展设计，不能当作已上线功能。

验证：执行相关 93 个测试及生产构建通过；Windows 的真实 Electron 测试创建 5 个隐藏窗口、10 个后台服务，并完成 60 条普通命令，确认窗口关闭隔离与最终资源计数归零。诊断入口为 `node scripts/diagnostics/execution-smoke.cjs`，报告位于 `.tmp/execution-smoke/report.json`。真实进程树测试需正常 Windows 进程权限；受限环境禁止停止进程树时会保留 unknown 状态，不通过只杀父进程伪装清理成功。
