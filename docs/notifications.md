...
推送配置入口：**设置 → 通知与外部推送**。通知通过操作系统或外部接收工具显示；右下角常驻的**消息中心**可回看编辑器提示和当前工作区的任务事件。简短操作提示保留在底部，并在设置或弹层打开时调整显示位置。

先开启“系统通知”，点击“推荐提醒”或“仅任务结果”，再点“保存更改”。通知设置与其他设置使用同一个保存入口，切换分类保留未保存内容。

“测试系统通知”直接调用操作系统，不保存设置，也不受其他 Webhook 草稿影响。测试不改变系统通知开关；开关关闭时不会自动提醒。系统接收测试请求后，是否展示横幅仍受通知权限、勿扰模式等影响。

“选择指定事件”提供中文勾选列表，覆盖 Agent 任务、计划、索引与素材、应用与连接、日常状态。“高级：自定义事件规则”供扩展模块使用，普通配置无需填写事件名。
...
渲染进程的 `editorEvents` 与主进程的 `mainEditorEvents` 接收统一格式的事件，由主进程 `NotificationService` 分发到系统通知与 Webhook。事件总线和通道接口独立，可继续添加事件来源、订阅者和其他推送实现。
...
普通状态只进入事件总线；完成、失败、等待输入等需要关注的事件按配置推送。取消任务不会被误报为完成。系统通知首次使用时默认启用、仅窗口在后台时提醒、静音；已有用户保存的开关不会自动改动。Webhook 默认没有配置，新建时关闭。勾选“包含日常状态变化”并选中信息级别后，通道也会接收匹配的普通状态。原有 Toast 作为日常状态事件处理，不会默认重复推送。

每个通道独立按事件名、级别筛选。`*` 匹配全部，`agent.*` 匹配 Agent，`index.completed` 精确匹配。相同事件默认冷却 15 秒。服务内部保留最多 200 条摘要。点击系统通知会打开对应的 Agent 会话（会话仍存在时）。

右下角保留常驻“消息中心”入口，即使没有消息或提示已消失也可打开。“编辑器消息”展示原有提示记录；“任务事件”展示当前工作区的事件摘要，支持未读筛选、标记已读、跳转任务和清空。历史变化只发送失效通知，界面通过现有的窗口/工作区鉴权接口重新读取，不广播其他工作区的记录。消息中心是历史查看面板，不是通知推送通道；不会恢复已移除的“应用内通知”设置，也不会额外生成弹窗。

## 通用 Webhook

先在接收消息的软件中创建机器人或 Webhook，再把接收方提供的地址填进“接收地址”。它不是普通网页 URL。按需设置请求头和 JSON 正文模板，使用该通道的“保存并发送测试消息”检查。支持最多 5 个通道。未启用的通道可以暂时留空地址；启用或测试前必须填写。界面会指出缺少地址、请求头 JSON 无效等具体问题。

接收方如果要求特定消息结构，可修改模板或通过自己的自动化服务转发。
...
消息仅含事件摘要，不自动携带代码、提示词、工具参数、原始错误、工作区路径或会话标识。新增事件生产者也应遵守这一约定。
...
Windows/Linux 系统通知不需要 Apple 开发者账号；系统权限、勿扰模式等会影响展示。macOS 的分发签名要求独立于该模块，当前 Windows 环境未验证 macOS 系统通知。Webhook 不依赖 Apple 开发者账号。平台说明见 [Electron 通知文档](https://www.electronjs.org/docs/latest/tutorial/notifications)。
