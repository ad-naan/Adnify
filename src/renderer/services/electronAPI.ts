...
function createGroupedAPI() {
  ...
  return {
    ...
    session: {
      ...
      loadPlans: () => raw.sessionLoadPlans(),
      upsertPlan: (plan: unknown) => raw.sessionUpsertPlan(plan),
      deletePlan: (planId: string) => raw.sessionDeletePlan(planId),
    ...
    settings: {
      ...
      captureDiagnostics: (options: Parameters<typeof raw.captureDiagnostics>[0]) => raw.captureDiagnostics(options),
