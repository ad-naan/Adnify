 ...
 * - 提供统一 API 给 UI 层
 *
 * 普通命令由主进程执行服务管理；此服务展示作业输出并承载显式交互会话。
 */
...
export interface TerminalExitEvent {
  ...
  seq: number;
  occurredAt: number;
  reason: 'process_exit' | 'killed_by_user' | 'remote_close' | 'idle_reclaimed';
  message?: string;
}
      ...
      (event: TerminalExitEvent) => {
        ...
        if (xterm?.terminal) {
          xterm.terminal.write(
            `\r\n\x1b[33m[${event.message || `Process exited with code ${exitCode}`}]\x1b[0m\r\n`,
          );
        }
