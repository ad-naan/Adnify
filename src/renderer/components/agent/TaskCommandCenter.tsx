...
function RenameOverlay({
  ...
  >
    <input
      ...
      aria-label={language === 'zh' ? '任务名称' : 'Task name'}
    ...
    />
    <button type="submit" disabled={!value.trim()} title={language === 'zh' ? '保存' : 'Save'} aria-label={language === 'zh' ? '保存' : 'Save'} className="flex h-7 w-7 items-center justify-center rounded-md text-accent hover:bg-accent/10 focus-visible:ring-2 focus-visible:ring-accent/45 disabled:opacity-35"><Check className="h-3.5 w-3.5" /></button>
    <button type="button" onClick={onCancel} title={language === 'zh' ? '取消' : 'Cancel'} aria-label={language === 'zh' ? '取消' : 'Cancel'} className="flex h-7 w-7 items-center justify-center rounded-md text-text-muted hover:bg-surface-hover hover:text-text-primary focus-visible:ring-2 focus-visible:ring-accent/45"><X className="h-3.5 w-3.5" /></button>
  </form>
}
  ...
  return <div>
    <div className="group relative" style={{ paddingLeft: `${node.depth * 17}px` }}>
      ...
      {node.threadId && <div className="absolute right-1 top-2 flex items-center gap-0.5">
        {actionsOpen && <ThreadActionStrip language={language} onRename={() => { setActionsOpen(false); setRenameValue(node.title); setRenaming(true) }} onAction={action => { setActionsOpen(false); onReference(node.threadId!, action) }} onDelete={() => { setActionsOpen(false); onDelete(node) }} deleteLabel={language === 'zh' ? '删除子会话' : 'Delete sub-thread'} />}
...
function TaskGroupCard({
  ...
  return <section className={`relative overflow-hidden rounded-lg transition-colors ${containsCurrent ? 'bg-accent/[0.09]' : group.status === 'waiting' ? 'bg-amber-400/[0.055]' : 'hover:bg-surface/[0.22]'}`}>
    <div className="group relative flex min-h-12 items-center gap-2.5 py-2 pl-3 pr-2">
      ...
      {actionsOpen && rootNode?.threadId && <ThreadActionStrip language={language} onRename={() => { setActionsOpen(false); setRenameValue(group.title); setRenaming(true) }} onAction={action => { setActionsOpen(false); onReference(rootNode.threadId!, action) }} onDelete={() => { setActionsOpen(false); onDelete?.() }} deleteLabel={language === 'zh' ? '删除任务' : 'Delete task'} />}
