...
export default function EmptyChatSuggestions({ onSelectSuggestion }: EmptyChatSuggestionsProps) {
    ...
    const latestCommitMsg = latestCommit?.message || (t('emptyChatSuggestions.noCommits', asLanguage(language)))
            ...
            >
                ...
                <div className="flex items-center gap-4 bg-gradient-to-r from-[rgb(var(--accent)/0.08)] to-[rgb(var(--accent)/0.02)] p-5 rounded-2xl mb-8 border border-[rgb(var(--accent)/0.1)]">
                    ...
                    <div>
                        <h2 className="text-[15px] font-semibold text-[rgb(var(--text-primary))] mb-1.5 tracking-tight">
                            {t('emptyChatSuggestions.whatAreWePushing', asLanguage(language))}
                        </h2>
                        <p className="text-[12px] text-[rgb(var(--text-secondary))] leading-relaxed max-w-[240px]">
                            {t('emptyChatSuggestions.iAmReadyTo', asLanguage(language))}
                        </p>
                    </div>
                </div>
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-3 px-1">
                        <h3 className="text-[13px] font-bold text-[rgb(var(--text-primary))]">{t('emptyChatSuggestions.projectInsights', asLanguage(language))}</h3>
                        <button className="flex items-center gap-1.5 text-[11px] text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text-primary))] transition-colors">
                            ...
                            <span>{t('refresh', asLanguage(language))}</span>
                        </button>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl p-3 flex items-start gap-3 hover:bg-[rgb(var(--surface))] transition-colors">
                            ...
                            <div className="min-w-0">
                                <div className="text-[11px] text-[rgb(var(--text-muted))] mb-0.5">{t('emptyChatSuggestions.branch', asLanguage(language))}</div>
                        ...
                        <div className="bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl p-3 flex items-start gap-3 hover:bg-[rgb(var(--surface))] transition-colors">
                            ...
                            <div className="min-w-0">
                                <div className="text-[11px] text-[rgb(var(--text-muted))] mb-0.5">{t('emptyChatSuggestions.workspace', asLanguage(language))}</div>
                                <div className="text-[13px] font-medium text-[rgb(var(--text-primary))] truncate">
                                    /{workspacePath ? getFileName(workspacePath) : (t('emptyChatSuggestions.none', asLanguage(language)))}
                                </div>
                            </div>
                        </div>
                        {/* Changes */}
                        <div className="bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl p-3 flex items-start gap-3 hover:bg-[rgb(var(--surface))] transition-colors">
                            ...
                            <div className="min-w-0">
                                <div className="text-[11px] text-[rgb(var(--text-muted))] mb-0.5">{t('emptyChatSuggestions.changes', asLanguage(language))}</div>
                                <div className="text-[13px] font-medium text-emerald-500">{changedFilesCount} {t('emptyChatSuggestions.files', asLanguage(language))}</div>
                            </div>
                        </div>
                        {/* Commits */}
                        <div className="bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl p-3 flex items-start gap-3 hover:bg-[rgb(var(--surface))] transition-colors">
                            ...
                            <div className="min-w-0">
                                <div className="text-[11px] text-[rgb(var(--text-muted))] mb-0.5">{t('emptyChatSuggestions.latestCommit', asLanguage(language))}</div>
                                <div className="text-[13px] font-medium text-[rgb(var(--text-primary))] truncate">
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-3 px-1">
                        <h3 className="text-[13px] font-bold text-[rgb(var(--text-primary))]">{t('emptyChatSuggestions.commandCenter', asLanguage(language))}</h3>
                        <div className="text-[11px] text-[rgb(var(--text-muted))] flex items-center gap-1">
                            <Zap className="w-3 h-3" />
                            {t('emptyChatSuggestions.quickExecute', asLanguage(language))}
                        </div>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                        <button onClick={() => onSelectSuggestion('/explain')} className="flex flex-col items-center justify-center py-3 px-1 bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl hover:bg-[rgb(var(--accent)/0.1)] hover:border-[rgb(var(--accent)/0.3)] transition-colors group">
                            ...
                            <div className="text-[10px] text-[rgb(var(--text-muted))] group-hover:text-[rgb(var(--text-secondary))]">{t('emptyChatSuggestions.explainCode', asLanguage(language))}</div>
                        </button>
                        <button onClick={() => onSelectSuggestion('/generate')} className="flex flex-col items-center justify-center py-3 px-1 bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl hover:bg-[rgb(var(--accent)/0.1)] hover:border-[rgb(var(--accent)/0.3)] transition-colors group">
                            ...
                            <div className="text-[10px] text-[rgb(var(--text-muted))] group-hover:text-[rgb(var(--text-secondary))]">{t('emptyChatSuggestions.generate', asLanguage(language))}</div>
                        </button>
                        <button onClick={() => onSelectSuggestion('/refactor')} className="flex flex-col items-center justify-center py-3 px-1 bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl hover:bg-emerald-500/10 hover:border-emerald-500/30 transition-colors group">
                            ...
                            <div className="text-[10px] text-[rgb(var(--text-muted))] group-hover:text-[rgb(var(--text-secondary))]">{t('emptyChatSuggestions.refactor', asLanguage(language))}</div>
                        </button>
                        <button onClick={() => onSelectSuggestion('/fix')} className="flex flex-col items-center justify-center py-3 px-1 bg-[rgb(var(--surface)/0.6)] border border-[rgb(var(--border)/0.5)] rounded-xl hover:bg-orange-500/10 hover:border-orange-500/30 transition-colors group">
                            ...
                            <div className="text-[10px] text-[rgb(var(--text-muted))] group-hover:text-[rgb(var(--text-secondary))]">{t('emptyChatSuggestions.fixBugs', asLanguage(language))}</div>
                        </button>
                    </div>
                </div>
                ...
                <div className="flex flex-col">
                    <h3 className="text-[13px] font-bold text-[rgb(var(--text-primary))] mb-3 px-1 shrink-0">{t('emptyChatSuggestions.startFromHere', asLanguage(language))}</h3>
                    <div className="flex flex-col gap-2 pb-6 px-1">
                        {[
                        {
                            ...
                            title: t('emptyChatSuggestions.analyzeCurrentProject', asLanguage(language)),
                            desc: t('emptyChatSuggestions.analyzeStructureAndDependencies', asLanguage(language)),
                            prompt: t('emptyChatSuggestions.pleaseAnalyzeTheCurrent', asLanguage(language))
                        },
                        {
                            icon: <Box className="w-4 h-4 text-emerald-500" />, iconBg: 'bg-emerald-500/10',
                            title: t('emptyChatSuggestions.generateModulePlan', asLanguage(language)),
                            desc: t('emptyChatSuggestions.generateImplementationPlanBased', asLanguage(language)),
                            prompt: t('emptyChatSuggestions.pleaseGenerateAnImplementation', asLanguage(language))
                        },
                        {
                            icon: <FileText className="w-4 h-4 text-emerald-500" />, iconBg: 'bg-emerald-500/10',
                            title: t('emptyChatSuggestions.addCommentsToFile', asLanguage(language)),
                            desc: t('emptyChatSuggestions.autoGenerateCommentsTo', asLanguage(language)),
                            prompt: t('emptyChatSuggestions.pleaseAddComprehensiveComments', asLanguage(language))
                        },
                        {
                            icon: <AlertCircle className="w-4 h-4 text-orange-500" />, iconBg: 'bg-orange-500/10',
                            title: t('emptyChatSuggestions.findHiddenIssues', asLanguage(language)),
                            desc: t('emptyChatSuggestions.scanForPotentialRisks', asLanguage(language)),
                            prompt: t('emptyChatSuggestions.pleaseScanTheCurrent', asLanguage(language))
                        }
                    ].map((item, idx) => (
