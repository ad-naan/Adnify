      ...
      {content.sections.map(section => {
        ...
        return <section key={section.id} className={`overflow-hidden rounded-xl border border-border/55 bg-surface/[0.07] ${compact ? '' : 'shadow-[0_1px_0_rgba(0,0,0,0.02)]'}`}>
          <div className={`flex items-start gap-3 border-b border-border/40 ${compact ? 'px-3 py-2.5' : 'px-4 py-3.5'}`}>
            ...
            <div className="min-w-0"><h3 className={`${compact ? 'text-[10px]' : 'text-[12px]'} font-semibold text-text-primary`}>{section.title}</h3>{section.description && <p className={`mt-1 text-text-muted ${compact ? 'text-[9px] leading-4' : 'text-[10px] leading-5'}`}>{section.description}</p>}</div>
          </div>
          {section.items.length > 0 && <div className="divide-y divide-border/35">
            {section.items.map(item => <div key={item.id} className={`flex items-start gap-3 ${compact ? 'px-3 py-2.5' : 'px-4 py-3'}`}>
              ...
              <div className="min-w-0 flex-1"><div className={`${compact ? 'text-[10px]' : 'text-[11px]'} font-medium text-text-secondary`}>{item.title}</div>{item.description && <p className={`mt-1 text-text-muted ${compact ? 'text-[9px] leading-4' : 'text-[10px] leading-5'}`}>{item.description}</p>}</div>
            </div>)}
          </div>}
        </section>
      })}
