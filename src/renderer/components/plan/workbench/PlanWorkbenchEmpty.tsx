  ...
  return <div className="flex h-full min-h-[360px] items-center justify-center px-5 pb-24">
    <div className="w-full max-w-[360px]">
      <div className="flex flex-col items-center text-center">
        ...
        <h2 className="text-[13px] font-semibold text-text-primary">{t('planWorkbenchEmpty.startWithAnObjective', asLanguage(language))}</h2>
        <p className="mt-2 max-w-[300px] text-[10px] leading-[18px] text-text-muted">{t('planWorkbenchEmpty.describeWhatYouWant', asLanguage(language))}</p>
      </div>

      {recent.length > 0 && <div className="mt-7">
        <div className="mb-2 flex items-center justify-between px-1"><span className="text-[11px] font-medium text-text-muted">{t('planWorkbenchEmpty.recentPlans', asLanguage(language))}</span><button onClick={onOpenHistory} className="inline-flex items-center gap-1 text-[11px] text-text-muted hover:text-accent"><History className="h-3 w-3" />{t('common.all', asLanguage(language))}</button></div>
      ...
      {recent.length === 0 && <div className="mt-6 text-center text-[10px] text-text-muted/65">{t('planWorkbenchEmpty.createdPlansWillBe', asLanguage(language))}</div>}
    </div>
  </div>
}
