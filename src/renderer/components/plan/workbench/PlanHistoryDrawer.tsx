...
const statusText = (status: PlanHistoryEntry['status'], language: string) => {
  if (!status) return t('planHistoryDrawer.conversation', asLanguage(language))
...
function groupName(timestamp: number, language: string) {
  ...
  if (startValue === startToday) return t('planHistoryDrawer.today', asLanguage(language))
  if (startToday - startValue < 7 * 86_400_000) return t('planHistoryDrawer.last7Days', asLanguage(language))
  return t('planHistoryDrawer.earlier', asLanguage(language))
}
  ...
  return <div className="absolute inset-0 z-40 bg-background">
    <aside ref={panelRef} className="absolute inset-0 flex flex-col bg-background">
      <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/45 px-3.5">
        <div className="min-w-0 flex-1"><div className="text-[11px] font-semibold text-text-primary">{t('common.planHistory', asLanguage(language))}</div><div className="mt-1 text-[10px] text-text-muted">{t('planHistoryDrawer.separateFromAgentConversations', asLanguage(language))}</div></div>
        <button onClick={onClose} aria-label={t('closeTerminal', asLanguage(language))} className="rounded-md p-1.5 text-text-muted hover:bg-surface-hover hover:text-text-primary"><X className="h-3.5 w-3.5" /></button>
      </header>
      <div className="shrink-0 px-3.5 py-3">
        <label className="relative block"><Search className="absolute left-2.5 top-1/2 h-3 w-3 -translate-y-1/2 text-text-muted/55" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder={t('planHistoryDrawer.searchPlanHistory', asLanguage(language))} className="h-8 w-full rounded-md border border-border/50 bg-surface/[0.1] pl-8 pr-3 text-[11px] text-text-primary outline-none placeholder:text-text-muted/45 focus:border-accent/35" /></label>
      </div>
          ...
          <div className="divide-y divide-border/35 border-y border-border/35">
            {group.entries.map(entry => <div key={entry.id} className="group flex items-start gap-1">
              ...
              {confirmingId === entry.id ? <div className="flex shrink-0 items-center gap-1 self-center">
                <button onClick={() => { onDelete(entry); setConfirmingId(null) }} className="rounded px-1.5 py-1 text-[10px] font-medium text-red-400 hover:bg-red-500/10">{t('planHistoryDrawer.delete', asLanguage(language))}</button>
                <button onClick={() => setConfirmingId(null)} className="rounded px-1.5 py-1 text-[10px] text-text-muted hover:bg-surface-hover">{t('cancel', asLanguage(language))}</button>
              </div> : <button onClick={() => setConfirmingId(entry.id)} aria-label={t('planHistoryDrawer.deletePlanHistory', asLanguage(language))} className="shrink-0 self-center rounded p-1.5 text-text-muted opacity-0 hover:bg-red-500/10 hover:text-red-400 focus-visible:opacity-100 group-hover:opacity-100"><Trash2 className="h-3 w-3" /></button>}
            </div>)}
          </div>
        </section>) : <div className="flex min-h-48 flex-col items-center justify-center text-center"><OtterAsset asset="sleepyFace" className="h-12 w-12 object-contain opacity-75" /><div className="mt-3 text-[10px] font-medium text-text-secondary">{t('planHistoryDrawer.noMatchingPlans', asLanguage(language))}</div><div className="mt-1 text-[10px] text-text-muted">{t('planHistoryDrawer.tryAnotherSearch', asLanguage(language))}</div></div>}
      </div>
      <footer className="shrink-0 border-t border-border/40 p-3.5">
        ...
        >
          ...
          {t('planHistoryDrawer.startANewPlan', asLanguage(language))}
        </button>
        <div className="mt-2 text-center text-[10px] text-text-muted">{t('planHistoryDrawer.records', asLanguage(language), { length: entries.length })}</div>
      </footer>
    </aside>
  </div>
}
