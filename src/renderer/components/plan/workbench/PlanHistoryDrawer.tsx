    ...
    <aside ref={panelRef} className="absolute inset-0 flex flex-col bg-background">
          ...
          <div className="divide-y divide-border/35 border-y border-border/35">
            {group.entries.map(entry => <div key={entry.id} className="group flex items-start gap-1">
              <button onClick={() => { onSelect(entry); onClose() }} className="min-w-0 flex-1 py-3 text-left">
                <div className="grid grid-cols-[8px_minmax(0,1fr)_auto_auto] items-center gap-2.5"><span className={`h-1.5 w-1.5 rounded-full ${dotTone(entry)}`} /><div className="min-w-0 truncate text-[10px] font-medium text-text-secondary">{entry.title}</div><span className={`text-[8px] font-medium ${entry.status === 'completed' ? 'text-emerald-500' : entry.status === 'failed' ? 'text-red-400' : 'text-amber-500'}`}>{statusText(entry.status, language)}</span><span className="flex items-center gap-2 text-[8px] text-text-muted">{entry.taskCount !== undefined && <span>{entry.completedCount}/{entry.taskCount}</span>}<time>{new Date(entry.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></span></div>
              </button>
      ...
      <footer className="shrink-0 border-t border-border/40 p-3.5">
        <button
          ...
          className="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-md border border-border/55 text-[9px] font-medium text-text-secondary hover:border-accent/30 hover:bg-accent/[0.035] hover:text-accent"
        >
          <Plus className="h-3 w-3" />
          {language === 'zh' ? '开始新计划' : 'Start a new plan'}
        </button>
        <div className="mt-2 text-center text-[8px] text-text-muted">{language === 'zh' ? `共 ${entries.length} 条记录` : `${entries.length} records`}</div>
      </footer>
    </aside>
  </div>
}
