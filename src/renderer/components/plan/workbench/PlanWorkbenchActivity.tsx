...
export function PlanWorkbenchActivity({ activities, language }: { activities: PlanActivityItem[], language: Language }) {
  ...
  return <details className="plan-activity-history mt-5">
    <summary><ChevronDown size={14} /><span>{t('planWorkbenchActivity.activity', language)}</span><span className="ml-auto tabular-nums">{history.length}</span></summary>
    <div className="mb-2.5 flex items-center justify-between px-0.5"><span className="text-xs font-medium text-text-muted">{t('planWorkbenchActivity.activity', language)}</span>{completedTools.length > 0 && <button onClick={() => setToolsOpen(value => !value)} className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-text-secondary"><span>{completedTools.length} {t('planWorkbenchActivity.toolActions', language)}</span><ChevronDown className={`h-3 w-3 transition-transform ${toolsOpen ? 'rotate-180' : ''}`} /></button>}</div>
      ...
      {visibleEvents.reverse().map(activity => <article key={activity.id} className="relative pb-3.5 last:pb-0">
        ...
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <div className="text-xs font-medium leading-4 text-text-secondary">{activity.title}</div>
            {activity.detail && <p className="mt-1 line-clamp-3 text-xs leading-[17px] text-text-muted">{activity.detail}</p>}
          </div>
          <span className="shrink-0 text-xs text-text-muted">{stageLabel(activity.stage, language)}</span>
        </div>
      </article>)}
    </div>}
    {toolsOpen && completedTools.length > 0 && <div className="mt-2 divide-y divide-border/30 overflow-hidden rounded-lg border border-border/40 bg-surface/[0.06]">
      {completedTools.slice().reverse().map(activity => <div key={activity.id} className="flex items-center gap-2 px-2.5 py-2">
        ...
        <span className="min-w-0 flex-1 truncate text-xs text-text-muted">{activity.title}</span>
        {activity.detail && <span className="max-w-[45%] truncate text-xs text-text-muted">{activity.detail}</span>}
      </div>)}
    </div>}
  </details>
}
