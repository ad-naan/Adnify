...
const panelTitle = (stage: string, language: string) => ({
  requirements: t('planWorkbench.requirementReview', asLanguage(language)),
  plan: t('planWorkbench.planReview', asLanguage(language)),
  execution: t('planWorkbench.runCenter', asLanguage(language)),
  validation: t('planWorkbench.resultReview', asLanguage(language)),
})[stage] || (t('planWorkbench.planMode', asLanguage(language)))
  ...
  const visibleFocus = useMemo<PlanWorkbenchFocus | null>(() => {
    if (model.isProcessing && model.focus?.tone !== 'active') return {
      ...
      title: model.planningState === 'ready_to_create'
        ? (t('planWorkbench.creatingTheStructuredPlan', asLanguage(language)))
        : (t('planWorkbench.preparingTheNextAction', asLanguage(language))),
      detail: model.planningState === 'ready_to_create'
        ? (t('planWorkbench.confirmedRequirementsAreBecoming', asLanguage(language)))
        : (t('planWorkbench.modelAndToolActivity', asLanguage(language))),
      tone: 'active',
    }
    if (plan && model.canStart && !model.isProcessing) return {
      stage: 'plan',
      title: t('planWorkbench.planIsReady', asLanguage(language)),
      detail: `${model.tasks.length} ${t('planWorkbench.tasks', asLanguage(language))} · ${plan.executionMode === 'parallel' ? (t('planWorkbench.parallel', asLanguage(language))) : (t('planWorkbench.sequential', asLanguage(language)))}`,
      tone: 'info',
    }
    if (plan?.status === 'completed') return {
      stage: 'validation',
      title: t('planWorkbench.executionComplete', asLanguage(language)),
      detail: `${model.completedCount}/${model.tasks.length} ${t('common.tasksCompleted', asLanguage(language))}`,
  ...
  if (!model.hasSession) return <div className="plan-readable relative h-full bg-background">
    <div className="absolute right-3 top-3 z-10 flex items-center gap-1">
      <button
        ...
        aria-label={t('planWorkbench.startNewPlan', asLanguage(language))}
        title={t('planWorkbench.startNewPlan', asLanguage(language))}
      ...
      <button
        ...
        aria-label={t('common.planHistory', asLanguage(language))}
        title={t('common.planHistory', asLanguage(language))}
        className="rounded-md p-1.5 text-text-muted hover:bg-surface-hover hover:text-text-primary transition-colors"
      >
    ...
    <header className="shrink-0 border-b border-border/45 px-4 py-4">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-2 text-[11px] text-text-muted">{displayStage === 'execution' ? <>{model.stage === 'execution' && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />}{plan?.executionMode === 'parallel' ? (t('planWorkbench.parallelOrchestration', asLanguage(language))) : (t('planWorkbench.sequentialOrchestration', asLanguage(language)))}{model.stage === 'execution' && <span className="ml-auto tabular-nums">{t('planWorkbench.total', asLanguage(language))} {formatElapsed(elapsedSeconds * 1000)}</span>}</> : plan?.name || (t('planWorkbench.buildingTheBrief', asLanguage(language)))}</div>
        </div>
      ...
      <div className="px-3.5 pb-5 pt-3.5">
        ...
        {!model.isProcessing && visibleFocus && displayStage === 'requirements' && visibleFocus.stage === 'requirements' && <section className="border-b border-border/40 pb-3.5">
          <div className="flex items-center gap-2 text-[10px] font-medium text-text-muted">
            ...
            <span>{visibleFocus.tone === 'warning' ? (t('planWorkbench.attention', asLanguage(language))) : visibleFocus.tone === 'completed' ? (t('planWorkbench.complete', asLanguage(language))) : (t('planWorkbench.currentFocus', asLanguage(language)))}</span>
        ...
        {model.requestText && displayStage === 'requirements' && <section className="border-b border-border/35 py-3.5">
          <div className="flex items-center gap-2 text-[11px] font-medium text-text-muted"><FileText className="h-3 w-3" />{t('common.objective', asLanguage(language))}</div>
        ...
        {plan && model.canStart && displayStage === 'plan' && <section className="mt-3">
          <div className="text-[11px] font-medium text-text-muted">{t('planWorkbench.planSummary', asLanguage(language))}</div>
          <div className="mt-2 grid grid-cols-3 divide-x divide-border/40 rounded-lg border border-border/45 bg-surface/[0.08] py-2.5 text-center">
            <div><strong className="block text-[11px] font-semibold tabular-nums text-text-primary">{model.review?.taskCount || 0}</strong><span className="mt-1 block text-[10px] text-text-muted">{t('common.tasks', asLanguage(language))}</span></div>
            <div><strong className="block text-[11px] font-semibold tabular-nums text-text-primary">{model.review?.maxParallelism || 0}</strong><span className="mt-1 block text-[10px] text-text-muted">{t('common.maxParallel', asLanguage(language))}</span></div>
            <div><strong className="block text-[11px] font-semibold tabular-nums text-text-primary">{model.review?.declaredArtifacts || 0}</strong><span className="mt-1 block text-[10px] text-text-muted">{t('planWorkbench.artifacts', asLanguage(language))}</span></div>
          </div>
          <dl className="mt-2 divide-y divide-border/35 border-y border-border/35 text-[11px]">
            <div className="flex items-center justify-between py-2"><dt className="flex items-center gap-1.5 text-text-muted"><GitBranch className="h-3 w-3" />{t('planWorkbench.strategy', asLanguage(language))}</dt><dd className="font-medium text-text-secondary">{plan.executionMode === 'parallel' ? (t('planWorkbench.parallel2', asLanguage(language))) : (t('planWorkbench.sequential2', asLanguage(language)))}</dd></div>
            <div className="flex items-center justify-between py-2"><dt className="flex items-center gap-1.5 text-text-muted"><ShieldCheck className="h-3 w-3" />{t('planWorkbench.approvals', asLanguage(language))}</dt><dd className="font-medium text-text-secondary">{t('planWorkbench.perPermissionedTool', asLanguage(language))}</dd></div>
            <div className="flex items-center justify-between py-2"><dt className="flex items-center gap-1.5 text-text-muted"><Boxes className="h-3 w-3" />{t('planWorkbench.tokenBudget', asLanguage(language))}</dt><dd className="font-medium tabular-nums text-text-secondary">{model.review?.estimatedTokens ? model.review.estimatedTokens.toLocaleString() : '—'}</dd></div>
          </dl>
          {model.review && model.review.allocations.length > 0 && <div className="mt-3">
            <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-medium text-text-muted"><UserRoundCog className="h-3 w-3" />{t('planWorkbench.roleAndModelAllocation', asLanguage(language))}</div>
            <div className="divide-y divide-border/30 rounded-lg border border-border/40">
              {model.review.allocations.map(item => <div key={item.key} className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 px-2.5 py-2"><div className="min-w-0"><div className="truncate text-[11px] font-medium text-text-secondary">{item.role}</div><div className="mt-0.5 truncate text-[10px] text-text-muted">{providerConfigs[item.provider]?.displayName || BUILTIN_PROVIDERS[item.provider]?.displayName || item.provider} · {item.model}</div></div><span className="self-center text-[10px] tabular-nums text-text-muted">{item.taskCount} {t('planWorkbench.tasks2', asLanguage(language))}</span></div>)}
            </div>
          </div>}
          {model.review && <div className="mt-3">
            <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-medium text-text-muted"><AlertTriangle className="h-3 w-3" />{t('planWorkbench.structuralChecks', asLanguage(language))}</div>
            {model.review.risks.length ? <div className="space-y-1.5">{model.review.risks.map(risk => <div key={risk.id} className={`rounded-md border px-2.5 py-2 ${risk.severity === 'error' ? 'border-red-400/25 bg-red-400/[0.035]' : 'border-amber-400/25 bg-amber-400/[0.035]'}`}><div className={`text-[10px] font-medium ${risk.severity === 'error' ? 'text-red-400' : 'text-amber-500'}`}>{language === 'zh' ? risk.titleZh : risk.title}</div><div className="mt-1 text-[10px] leading-4 text-text-muted">{language === 'zh' ? risk.detailZh : risk.detail}</div></div>)}</div> : <div className="flex items-center gap-1.5 rounded-md border border-border/35 px-2.5 py-2 text-[10px] text-text-muted"><CheckCircle2 className="h-3 w-3 text-emerald-400" />{t('planWorkbench.noCyclesMissingDependencies', asLanguage(language))}</div>}
          </div>}
          <button onClick={startPlan} disabled={starting} className="mt-4 inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-md bg-accent text-[10px] font-medium text-white shadow-sm hover:bg-accent-hover disabled:opacity-50">{starting ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}{t('common.approveAndRun', asLanguage(language))}</button>
          <button onClick={revisePlan} className="mt-2 inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-md border border-border/60 text-[10px] font-medium text-text-secondary hover:bg-surface-hover"><RotateCcw className="h-3.5 w-3.5" />{t('planWorkbench.returnToRevise', asLanguage(language))}</button>
          <p className="mt-2 text-center text-[10px] leading-4 text-text-muted/65">{t('planWorkbench.permissionedToolsStillRequest', asLanguage(language))}</p>
        </section>}
        ...
        {displayStage === 'validation' && model.stage === 'validation' && validation && <section className="mt-4 border-t border-border/40 pt-3.5">
          <div className="flex items-center gap-2 text-[11px] font-medium text-text-muted">{validation.failed > 0 ? <TriangleAlert className="h-3 w-3 text-amber-400" /> : <CheckCircle2 className="h-3 w-3 text-emerald-400" />}{t('planWorkbench.deliveryResult', asLanguage(language))}</div>
          <div className="mt-2 grid grid-cols-3 divide-x divide-border/40 border-y border-border/35 py-2.5 text-center">
            <div><div className="text-[11px] font-semibold tabular-nums text-text-primary">{model.completedCount}/{model.tasks.length}</div><div className="mt-1 text-[10px] text-text-muted">{t('common.tasks', asLanguage(language))}</div></div>
            <div><div className={`text-[11px] font-semibold tabular-nums ${validation.failed ? 'text-red-400' : 'text-text-primary'}`}>{validation.failed}</div><div className="mt-1 text-[10px] text-text-muted">{t('common.failed', asLanguage(language))}</div></div>
            <div><div className="text-[11px] font-semibold tabular-nums text-text-primary">{validation.duration ? formatElapsed(validation.duration) : '—'}</div><div className="mt-1 text-[10px] text-text-muted">{t('planWorkbench.elapsed', asLanguage(language))}</div></div>
          </div>
          {validation.files.length > 0 && <div className="mt-2 text-[10px] leading-4 text-text-muted">{t('planWorkbench.plannedResources', asLanguage(language), { length: validation.files.length })}</div>}
          {plan?.validation?.status === 'accepted' ? <div className="mt-3 flex items-center gap-2 rounded-md border border-emerald-400/25 bg-emerald-400/[0.04] px-3 py-2.5 text-[11px] font-medium text-emerald-500"><CheckCircle2 className="h-3.5 w-3.5" />{t('planWorkbench.resultsAcceptedAndSaved', asLanguage(language))}</div> : <div className="mt-3 grid grid-cols-2 gap-2">
            <button onClick={requestValidationChanges} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-border/60 text-[11px] font-medium text-text-secondary hover:bg-surface-hover"><MessageSquareText className="h-3 w-3" />{t('common.requestChanges', asLanguage(language))}</button>
            <button onClick={acceptValidation} disabled={validation.failed > 0} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md bg-accent text-[11px] font-medium text-white hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-45"><Check className="h-3 w-3" />{t('planWorkbench.acceptResults', asLanguage(language))}</button>
          </div>}
          {validation.failed > 0 && plan?.validation?.status !== 'accepted' && <p className="mt-2 text-[10px] leading-4 text-amber-500">{t('planWorkbench.resolveOrRerunFailed', asLanguage(language))}</p>}
        </section>}
