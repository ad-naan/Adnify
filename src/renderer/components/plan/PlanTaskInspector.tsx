...
export const PlanTaskInspector = memo(function PlanTaskInspector({ task, tasks, language, disabled, onChange, onClose }: Props) {
  ...
  const executionOptions = useMemo(() => ([
    { value: 'general', label: t('planTaskInspector.general', asLanguage(language)) },
    { value: 'analysis-read-heavy', label: t('planTaskInspector.analysisReadHeavy', asLanguage(language)) },
    { value: 'write-heavy', label: t('planTaskInspector.writeHeavy', asLanguage(language)) },
    { value: 'approval-heavy', label: t('planTaskInspector.approvalHeavy', asLanguage(language)) },
  ]), [language])
  ...
  return <aside className="absolute inset-y-0 right-0 z-30 flex w-[380px] max-w-[92%] flex-col border-l border-border/55 bg-background/98 shadow-[-20px_0_42px_-30px_rgba(15,23,42,0.34)] backdrop-blur-sm">
    <header className="flex h-10 shrink-0 items-center gap-2 border-b border-border/45 px-3.5">
      ...
      <span className="min-w-0 flex-1 truncate text-[10px] font-semibold text-text-primary">{t('planTaskInspector.taskSetup', asLanguage(language))}</span>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3.5 py-3 custom-scrollbar">
      {disabled && <div className="mb-3 flex gap-2 rounded-md border border-amber-400/25 bg-amber-400/[0.035] px-2.5 py-2 text-[10px] leading-4 text-amber-500"><AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" />{t('planTaskInspector.setupIsLockedWhile', asLanguage(language))}</div>}

      <label className="block"><span className="text-[10px] font-medium text-text-muted">{t('common.taskName', asLanguage(language))}</span><input key={`${task.id}:title`} defaultValue={task.title} disabled={disabled} onBlur={event => event.target.value.trim() && onChange({ title: event.target.value.trim() })} className="mt-1.5 h-8 w-full rounded-md border border-border/55 bg-surface/[0.08] px-2.5 text-[11px] text-text-primary outline-none focus:border-accent/40 disabled:opacity-55" /></label>
      <label className="mt-3 block"><span className="text-[10px] font-medium text-text-muted">{t('planTaskInspector.objective', asLanguage(language))}</span><textarea key={`${task.id}:description`} defaultValue={task.description} disabled={disabled} onBlur={event => onChange({ description: event.target.value.trim() })} rows={4} className="mt-1.5 w-full resize-none rounded-md border border-border/55 bg-surface/[0.08] px-2.5 py-2 text-[11px] leading-4 text-text-primary outline-none focus:border-accent/40 disabled:opacity-55" /></label>

      <section className="mt-4 border-t border-border/40 pt-3">
        <label className="block"><span className="flex items-center gap-1.5 text-[11px] font-medium text-text-muted"><ClipboardCheck className="h-3.5 w-3.5" />{t('planTaskInspector.acceptanceCriteria', asLanguage(language))}</span><textarea key={`${task.id}:criteria`} defaultValue={(task.acceptanceCriteria || []).map(item => item.text).join('\n')} disabled={disabled} onBlur={event => onChange({ acceptanceCriteria: criteriaFromText(criteriaLines(event.target.value), task.acceptanceCriteria) })} rows={4} placeholder={t('planTaskInspector.oneObservableConditionPer', asLanguage(language))} className="mt-1.5 w-full resize-none rounded-md border border-border/55 bg-surface/[0.08] px-2.5 py-2 text-[12px] leading-5 text-text-primary outline-none focus:border-accent/40" /></label>
      </section>

      <section className="mt-4 border-t border-border/40 pt-3">
        <div className="mb-2 text-[10px] font-medium text-text-muted">{t('planTaskInspector.roleAndModel', asLanguage(language))}</div>
      ...
      <section className="mt-4 border-t border-border/40 pt-3">
        <div className="mb-2 flex items-center gap-1.5 text-[10px] font-medium text-text-muted"><Boxes className="h-3 w-3" />{t('planTaskInspector.scheduling', asLanguage(language))}</div>
      ...
      <section className="mt-4 border-t border-border/40 pt-3">
        <div className="mb-2 flex items-center gap-1.5 text-[10px] font-medium text-text-muted"><GitBranch className="h-3 w-3" />{t('planTaskInspector.dependencies', asLanguage(language))}</div>
        <div className="max-h-44 overflow-y-auto rounded-md border border-border/45 px-2.5 custom-scrollbar">
          {tasks.filter(candidate => candidate.id !== task.id).map(candidate => {
            ...
            return <label key={candidate.id} className="flex items-start gap-2 border-b border-border/30 py-2 last:border-0">
              ...
              <span className="min-w-0 flex-1"><span className="block truncate text-[10px] text-text-secondary">{candidate.title}</span>{createsCycle && <span className="mt-0.5 block text-[10px] text-amber-500">{t('planTaskInspector.wouldCreateACycle', asLanguage(language))}</span>}</span>
            </label>
          })}
          {tasks.length <= 1 && <div className="py-3 text-center text-[10px] text-text-muted">{t('planTaskInspector.noOtherTasks', asLanguage(language))}</div>}
        </div>
      </section>

      <section className="mt-4 border-t border-border/40 pt-3">
        <label className="block"><span className="flex items-center gap-1.5 text-[10px] text-text-muted"><FileInput className="h-3 w-3" />{t('planTaskInspector.inputResources', asLanguage(language))}</span><textarea key={`${task.id}:inputs`} defaultValue={(task.consumesFiles || []).join('\n')} disabled={disabled} onBlur={event => onChange({ consumesFiles: filesFromText(event.target.value) })} rows={3} placeholder={t('planTaskInspector.onePerLineOr', asLanguage(language))} className="mt-1.5 w-full resize-none rounded-md border border-border/55 bg-surface/[0.08] px-2.5 py-2 text-[10px] leading-4 text-text-primary outline-none focus:border-accent/40" /></label>
        <label className="mt-3 block"><span className="flex items-center gap-1.5 text-[10px] text-text-muted"><FileOutput className="h-3 w-3" />{t('planTaskInspector.expectedArtifacts', asLanguage(language))}</span><textarea key={`${task.id}:outputs`} defaultValue={(task.producesFiles || []).join('\n')} disabled={disabled} onBlur={event => onChange({ producesFiles: filesFromText(event.target.value) })} rows={3} placeholder={t('planTaskInspector.onePerLineOr', asLanguage(language))} className="mt-1.5 w-full resize-none rounded-md border border-border/55 bg-surface/[0.08] px-2.5 py-2 text-[10px] leading-4 text-text-primary outline-none focus:border-accent/40" /></label>
      </section>
    </div>
  </aside>
})
