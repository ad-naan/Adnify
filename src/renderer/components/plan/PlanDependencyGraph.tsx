...
export const PlanDependencyGraph = memo(function PlanDependencyGraph({
  ...
  return <div className="relative min-h-0 flex-1 overflow-auto bg-[radial-gradient(circle_at_1px_1px,rgb(var(--border)/0.3)_1px,transparent_0)] bg-[size:20px_20px] custom-scrollbar">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-3">
                  <h3 title={task.title} className="line-clamp-1 text-[13px] font-semibold leading-5 text-text-primary">{task.title}</h3>
                  <span className={`flex shrink-0 items-center gap-1 text-[11px] font-medium ${meta.tone}`}><Icon className={`h-3 w-3 ${task.status === 'running' && !waitingApprovalTaskIds.has(task.id) ? 'animate-spin' : ''}`} />{meta.label}</span>
                </div>
                <p className="mt-1 line-clamp-2 text-[12px] leading-4 text-text-secondary">{task.description}</p>
              </div>
            </div>

            <div className="mt-auto grid grid-cols-2 gap-x-4 gap-y-1.5 border-t border-border/45 pt-2.5 text-[11px]">
