...
describe('EventBus emotion feedback', () => {
  it('delivers structured emotion feedback events to subscribers', () => {
    const payload: EmotionFeedbackPayload = {
      ...
      messageKey: 'emotion.suggestion.terminalError',
      shortMessageKey: 'emotion.status.frustrated.1',
      actions: [{ id: 'ask_ai', labelKey: 'emotion.action.askAi', actionType: 'ask_ai' }],
      createdAt: Date.now(),
